...
function Portfolios() {
  ...
  return <Stack spacing={2}><Stack direction="row" justifyContent="space-between"><Typography variant="h4">Portfolios</Typography><Button component={Link} to="/portfolios/new" variant="contained">New portfolio</Button></Stack>{query.data.length === 0 ? <Paper sx={{ p: 3 }}>No portfolios yet.</Paper> : <List>{query.data.map(p => <ListItemButton key={p.id} component={Link} to={`/portfolios/${p.id}`}><ListItemText primary={p.name} secondary={`${p.baseCurrency} · ${p.costBasisMethod} · ${p.returnMethod}`} /></ListItemButton>)}</List>}</Stack>;
}
...
function NewPortfolio() {
  ...
  return <Stack spacing={2} maxWidth={520}><Typography variant="h4">New portfolio</Typography><TextField label="Name" value={name} onChange={e => setName(e.target.value)} /><TextField label="Base currency" value={currency} onChange={e => setCurrency(e.target.value.toUpperCase())} inputProps={{ maxLength: 3 }} /><Button variant="contained" disabled={!name || mutation.isPending} onClick={() => mutation.mutate()}>Create</Button>{mutation.isError && <Alert severity="error">{mutation.error.message}</Alert>}</Stack>;
}
