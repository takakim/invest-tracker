...
- Native Historical Price Backfill, Corporate Actions Enrichment & AI Reasoning Control (PR #74):
  - **Native Historical Price Backfill**: Dense multi-year price synchronization for all instruments ever traded in a portfolio (both currently held and liquidated/sold positions) prioritizing high-throughput Yahoo Finance chart bars. Automatically triggered on portfolio history queries (`hasSufficientHistoricalCoverage`) and following broker CSV imports.
  - REST API: `POST /api/v1/portfolios/{id}/history/backfill?range={range}` with full OpenAPI contract updates.
  - Frontend UI: Interactive "Sync History" action button on `PortfolioHistoryCard.tsx` with loading feedback and TanStack Query cache invalidation.
  - **Corporate Actions Multi-Instrument Enrichment**: Support for corporate reorganizations and splits producing multiple resulting instruments (e.g. RGL, NVDA, SMCI).
  - **AI Reasoning Control & Formatting Leak Fix**:
    - Introduced `reasoning_effort: "none"` configuration in `LmStudioGateway`, `AiProperties`, and `application.yml`, eliminating the 150-second token-exhausting chain-of-thought loop on local reasoning LLMs (such as Qwen 3.5 9B in LM Studio) and lowering inference latency to ~11–16 seconds.
    - Updated `AiEvaluationService.extractJson` to strip `<think>...</think>` tags and reasoning prefixes, and added `sanitizeSummary` to prevent raw thinking traces from leaking into `executiveSummary`.
    - Multi-currency qualification in evaluation prompt (cost basis and current price clearly labeled in portfolio base currency `GBP`, with native currency price context).
  - **Reusable Python CLI Suite (`scripts/`)**:
    - `audit_ai_evaluations.py`: Live API evaluation auditor with anomaly detection and `--re-evaluate` trigger.
    - `test_llm_completion.py`: Standalone LLM completion benchmark and parameter tester.
    - Documented in `scripts/README.md` and `AGENTS.md`.
  - **Verification**: 914 backend tests and 135 frontend tests passing with 90.07% JaCoCo branch coverage (mandatory >=90% gate met), 0 OWASP CVEs, and 0 npm vulnerabilities.
